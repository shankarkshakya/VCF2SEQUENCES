...
avr_tajima_mat <- matrix(NA, nrow = length(mybin), ncol = 3)
rownames(avr_tajima_mat) <- names(mybin)
colnames(avr_tajima_mat) <- c("Tajima's D", "Pvalue", "Segregating sites")
avr_tajima_mat
...
for (i in 1:length(control_mybin)){
  ...
  control_tajima_mat[i,4] <- ncol(mybin[[i]])
}
